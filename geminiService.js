 ...
 *  1. Prompt Engineering      – Structured, role-based prompts with clear output schemas
 *  2. Context-Aware Generation – Resume + JD fed together for contextual reasoning
 *  3. Semantic Analysis        – LLM understands meaning, not just keywords
 *  4. LLM-Based Reasoning      – Chain-of-thought style explanations in every response
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const DEFAULT_MODEL_CANDIDATES = [
  ...
  "gemini-3.1-flash-lite-preview", // Much higher rate limits
  "gemini-2.5-flash", 
...
/**
 * Core helper: Send a prompt to Gemini and return parsed JSON.
 * Handles retries, JSON extraction from markdown fences, and errors.
 */
async function callGemini(prompt, expectJson = true) {
  ...
  try {
    ...
    // 🔥 NEW STEP 1: Aggressive cleaning of markdown and whitespace
    let cleanText = text.replace(/```json/gi, "").replace(/```/g, "").trim();

    try {
      // 🔥 NEW STEP 2: Try direct parse first 
      // This is the cleanest way and works best with responseMimeType: "application/json"
      const parsed = JSON.parse(cleanText);
      console.log("✅ JSON parsed successfully");
      return parsed;
    } catch (initialError) {
      // 🔥 NEW STEP 3: Fallback - Extract ONLY the JSON part if the AI added extra text
      ...
      if (jsonStart !== -1 && jsonEnd !== -1) {
        ...
        try {
          ...
          // Log a snippet of the failed JSON to the console to debug "Position 219"
...
// ═══════════════════════════════════════════════════════════════════════════════
// FEATURE 1: Resume Parsing & Structured Data Extraction
// GenAI Concept: Prompt Engineering + Semantic Analysis
// ═══════════════════════════════════════════════════════════════════════════════
async function extractResumeData(resumeText) {
  /**
   * PROMPT ENGINEERING STRATEGY:
   * - Role-based prompting: "You are an expert HR analyst..."
   * - Explicit JSON schema definition to enforce structured output
   * - Semantic extraction: asks for meaning/context, not just raw keywords
   * - Chain-of-thought: asks Gemini to infer things not explicitly stated
   */
  const prompt = `
...
// ═══════════════════════════════════════════════════════════════════════════════
// FEATURE 2: Semantic Resume–JD Matching
// GenAI Concept: Semantic Analysis + Context-Aware Generation
// ═══════════════════════════════════════════════════════════════════════════════
async function semanticMatch(resumeText, jobDescription) {
  /**
   * PROMPT ENGINEERING STRATEGY:
   * - Dual-context input: resume + JD analyzed together
   * - Semantic matching: looks for conceptual equivalents, not just string matches
   *   e.g., "React.js" matches "React", "UI development", "component-based frontend"
   * - Reasoning-chain: each gap/match includes WHY it matters for this specific role
   */
  const prompt = `
...
// ═══════════════════════════════════════════════════════════════════════════════
// FEATURE 3: ATS Score Generation (AI-Driven)
// GenAI Concept: LLM-Based Reasoning + Semantic Analysis
// ═══════════════════════════════════════════════════════════════════════════════
async function generateATSScore(resumeText, jobDescription, matchData) {
  /**
   * PROMPT ENGINEERING STRATEGY:
   * - Multi-factor scoring rubric defined in prompt
   * - Weighted components: ATS parsing, keyword coverage, semantic relevance, formatting
   * - Reasoning chain: Gemini must justify every component score
   * - Prior context injection: match data passed to avoid redundant computation
   */
  const prompt = `
...
// ═══════════════════════════════════════════════════════════════════════════════
// FEATURE 4: AI Resume Improvement (LLM Reasoning)
// GenAI Concept: Context-Aware Generation + Prompt Engineering
// ═══════════════════════════════════════════════════════════════════════════════
async function improveResume(resumeData, jobDescription) {
  /**
   * PROMPT ENGINEERING STRATEGY:
   * - Role: "You are a professional resume writer and career coach"
   * - Transformation task: BEFORE → AFTER format
   * - Impact-driven language directive: STAR method, quantified results
   * - JD-alignment: improvements tailored to the specific job context
   */
  const prompt = `
...
// ═══════════════════════════════════════════════════════════════════════════════
// FEATURE 5: AI Suggestions with Reasoning (Key Feature)
// GenAI Concept: LLM-Based Reasoning + Chain-of-Thought
// ═══════════════════════════════════════════════════════════════════════════════
async function generateSuggestions(resumeData, matchData, atsScore) {
  /**
   * PROMPT ENGINEERING STRATEGY:
   * - Consultant persona: gives actionable, prioritized advice
   * - Reasoning required for every suggestion (not just "add skills")
   * - Priority scoring: helps candidate focus on high-ROI changes
   * - Category tagging: organizes suggestions by type
   */
  const prompt = `
...
async function generateCoverLetter(resumeData, jobDescription, matchData) {
  /**
   * PROMPT ENGINEERING STRATEGY:
   * - Full context injection: candidate profile + JD + semantic match data
   * - Human-like writing directive: avoid corporate clichés, be authentic
   * - Structural guidance: hook → body → call-to-action
   * - Personalization layer: references specific company signals from JD
   */
  const prompt = `
...
// Runs all features in optimal order, passing context between stages
// ═══════════════════════════════════════════════════════════════════════════════
// Helper to prevent hitting rate limits on the Free Tier
